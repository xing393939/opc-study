...
1. block storage（容量16T）：
  ...
  * 特点1：io1/io2 现在支持Multi-Instance-Attach
  * 特点2：st1和sc1不能作为启动卷，其他包括上一代的磁介质都可以
  * EBS加密卷，卷与实例之间的传输也是加密的
  * EBS卷可以不停机的更改类型和增加size，除了磁介质；也不能减小size
  * 在EC2 Dashboard可以设置ebs默认开启加密
1. file storage：
  * Amazon Elastic File System：50w IOPS，多ec2共享，容量不限。pay-as-you-use
  * Amazon FSx for Windows：network filesystem for Windows，容量64T
  * Amazon FSx For Lustre：high-performance，容量不限；可和S3高度集成；适合 HPC(high-performance computing)
...
1. object storage：S3，如果出现file storage则不能选S3
  ...
  * Glacier：>90天，取回时间1m~12h；不能被Athena查询
...
1. Auto Scaling Group scaling 策略
  ...
  * scheduled scaling
  * 特点1：scaling activity，先terminate unhealthy instance，在起新的instance；rebalancing activity 则相反
...
#### RDS、Aurora、DynamoDB
1. RDS failing over：Amazon RDS points CNAME to the standby, promoted it an new primary
...
1. Aurora failover：
  ...
  1. else，在相同AZ重建一个实例
1. Aurora 可跨5个region建只读副本（延迟在1秒内）；双写功能不能跨region。
...
1. 备份还原：
  1. RDS 需开启自动备份（间隔5分钟），还原的时候选择还原点（snapshot）创建一个新实例
  1. Aurora 自动备份是默认开启的，可设置保留时间（1~35天），可恢复到保留时间内的任意时间点，可选择创建一个新实例（需几小时）或直接回溯（需几分钟）
...
1. SQS FIFO support 3000 messages per second with batching, or up to 300 messages per second；可以消息分组
1. SNS 也能触发lambda
...
1. ALB、CLB有安全组；NLB、NAT Gateway没有安全组
...
1. EC2 Default instances的母机是不确定的，Dedicated instances的母机是属于同一账号的；Default instances和Dedicated instances可以互相切换
...
1. IAM 三驾马车：Organization SCP、IAM permission boundaries、IAM permissions
