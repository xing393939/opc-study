...
* RDS fail-over：Amazon RDS points CNAME to the standby, promoted it an new primary
* Aurora fail-over：
...
* RDS SQL Server 不支持 IAM auth，安全连接的方法：
...
1. ALB、CLB有安全组；NLB、NAT Gateway没有安全组；安全组只有allow rule，没有deny rule
...
#### EC2
1. ec2：不是running状态无需付费，但是stopping到hibernate状态需要；预留实例terminated状态也要付费
