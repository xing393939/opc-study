...
1. 存储相关：
  1. block storage（容量16T）：
    ...
    1. Instance Store：200w IOPS，关机数据丢失；hibernate数据也会丢失
  ...
  1. file storage：
    ...
    1. Amazon FSx For Lustre：high-performance，容量不限；可和S3高度集成；适合 HPC(high-performance computing)
    1. EFS的模式：性能模式（GP、MAX IO）、吞吐量模式（Provisioned、Bursting）、存储模式（Standard、Infrequent Access）
  1. object storage：S3，如果出现file storage则不能选S3
    ...
    1. Glacier：>90天，取回时间1m~12h；不能被Athena查询
    ...
    1. 特点3：S3 is strongly consistent for all GET, PUT and LIST operations
  1. EBS 的优点：
...
1. Auto Scaling Group scaling 策略
  ...
  1. scheduled scaling
  1. 特点1：scaling activity，先terminate unhealthy instance，在起新的instance；rebalancing activity 则相反
